...
app.use((req, res, next) => {
  logLineAsync(`[${PORT}] render page, original url=${req.originalUrl}`, logPath);
  next();
});
...
app.get('/validate', (req, res) => {
  ...
  if (isFormDataValid(formData)) {
    ...
    res.send(getHomePage(formData, getErrorText(formData)));
  }
});

app.get('/success', (req, res) => {
  res.send(getPageSuccess(storeData));
});
